...
def resolve_dataset_loader(dataset_name):
    """Return the loader class for a supported dataset name."""
    loader_map = {
        "UBFC-rPPG": ("UBFCrPPGLoader", "UBFCrPPGLoader"),
        "PURE": ("PURELoader", "PURELoader"),
        "SCAMPS": ("SCAMPSLoader", "SCAMPSLoader"),
        "MMPD": ("MMPDLoader", "MMPDLoader"),
        "BP4DPlus": ("BP4DPlusLoader", "BP4DPlusLoader"),
        "BP4DPlusBigSmall": ("BP4DPlusBigSmallLoader", "BP4DPlusBigSmallLoader"),
        "UBFC-PHYS": ("UBFCPHYSLoader", "UBFCPHYSLoader"),
        "iBVP": ("iBVPLoader", "iBVPLoader"),
        "PhysDrive": ("PhysDriveLoader", "PhysDriveLoader"),
        "LADH": ("LADHLoader", "LADHLoader"),
        "SUMS": ("SUMSLoader", "SUMSLoader"),
        # Custom upload-style dataset support can be added by creating
        # dataset/data_loader/UploadLoader.py with class UploadLoader.
        "Upload": ("UploadLoader", "UploadLoader"),
    }

    if dataset_name not in loader_map:
        supported = ", ".join(sorted(loader_map.keys()))
        raise ValueError(
            f"Unsupported dataset '{dataset_name}'. Supported datasets: {supported}."
        )

    module_name, class_name = loader_map[dataset_name]

    if hasattr(data_loader, module_name):
        module = getattr(data_loader, module_name)
    else:
        try:
            module = importlib.import_module(f"dataset.data_loader.{module_name}")
        except ModuleNotFoundError as exc:
            if dataset_name == "Upload":
                raise ValueError(
                    "DATASET 'Upload' requires dataset/data_loader/UploadLoader.py "
                    "to be added to the toolbox first."
                ) from exc
            raise
...
if __name__ == "__main__":
    ...
    if config.TOOLBOX_MODE == "train_and_test":
        # train_loader
        train_loader = resolve_dataset_loader(config.TRAIN.DATA.DATASET)
        ...
        elif config.VALID.DATA.DATASET is not None:
            valid_loader = resolve_dataset_loader(config.VALID.DATA.DATASET)
    ...
    if config.TOOLBOX_MODE == "train_and_test" or config.TOOLBOX_MODE == "only_test":
        # test_loader
        test_loader = resolve_dataset_loader(config.TEST.DATA.DATASET)
        ...
        unsupervised_loader = resolve_dataset_loader(config.UNSUPERVISED.DATA.DATASET)
