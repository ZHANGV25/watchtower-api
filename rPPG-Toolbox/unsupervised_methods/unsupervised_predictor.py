...
def _safe_peak_hr(ppg_signal, fs):
    try:
        peak_hr = _calculate_peak_hr(ppg_signal, fs=fs)
    except Exception:
        return None
    if np.isnan(peak_hr) or np.isinf(peak_hr):
        return None
    return float(peak_hr)


def _save_extraction_results(config, method_name, clip_results):
    output_dir = config.UNSUPERVISED.OUTPUT_SAVE_DIR or "."
    os.makedirs(output_dir, exist_ok=True)

    fft_values = [item["fft_hr_bpm"] for item in clip_results if item["fft_hr_bpm"] is not None]
    peak_values = [item["peak_hr_bpm"] for item in clip_results if item["peak_hr_bpm"] is not None]

    payload = {
        "toolbox": "rPPG-Toolbox",
        "method": method_name,
        "dataset": config.UNSUPERVISED.DATA.DATASET,
        "fps": config.UNSUPERVISED.DATA.FS,
        "clip_count": len(clip_results),
        "summary": {
            "mean_fft_hr_bpm": float(np.mean(fft_values)) if fft_values else None,
            "mean_peak_hr_bpm": float(np.mean(peak_values)) if peak_values else None,
        },
        "clips": clip_results,
    }

    output_path = os.path.join(output_dir, f"rppg_toolbox_{method_name.lower()}_results.json")
...
def unsupervised_predict(config, data_loader, method_name):
    ...
    extract_only = getattr(config.UNSUPERVISED, "EXTRACT_ONLY", False)
    predict_hr_peak_all = []
    gt_hr_peak_all = []
    predict_hr_fft_all = []
    gt_hr_fft_all = []
    ...
    for _, test_batch in enumerate(sbar):
        ...
        for idx in range(batch_size):
            ...
            if extract_only:
                clip_results.append(
                    {
                        "filename": str(test_batch[2][idx]),
                        "chunk_id": str(test_batch[3][idx]),
                        "num_frames": int(len(BVP)),
                        "fft_hr_bpm": float(_calculate_fft_hr(BVP, fs=config.UNSUPERVISED.DATA.FS)),
                        "peak_hr_bpm": _safe_peak_hr(BVP, fs=config.UNSUPERVISED.DATA.FS),
                    }
                )
                continue
    ...
    if extract_only:
        _save_extraction_results(config, method_name, clip_results)
        return
